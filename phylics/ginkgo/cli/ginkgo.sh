...
then
	...
	do
    		...
    		#zgrep error: https://blog.nelhage.com/2010/02/a-very-subtle-bug/
            firstLineChr=$(${Z}grep --max 1 "chr" ${file} 2>/dev/null  |  tail -n+1  | cut -f1 )}
            if [ "${firstLineChr}" == "" ];
    		then
   		 ...
   		 # Bin reads
            echo "# -> binning reads"
    		${DIR_SCRIPTS}/binUnsorted ${DIR_GENOME}/${BINNING} ${NB_BINS} <(${Z}cat ${file}) `echo $(basename ${file}) | awk -F ".bed" '{print $1}'` ${file}_mapped
	done < ${DIR_CELLS_LIST}
...
${DIR_SCRIPTS}/process.R ${DIR_SCRIPTS} ${DIR_GENOME} ${DIR_INPUT} ${statFile} data ${SEGMENTATION} ${BINNING} ${CLUSTERING_LINKAGE} ${CLUSTERING_DISTANCE} ${COLOR} ${SEGMENTATION_REF}_mapped ${FACS} ${FILE_FACS} $( [[ $MASK_SEXCHRS == 1 ]] && echo 0 || echo 1 ) ${MASK_BADBINS}
...
for (( i=1; i<=$nbCols; i++ ));
do
  currCell=$(cut -f$i ${DIR_INPUT}/SegCopy 2>/dev/null | head -n 1 | tr -d '"')
  if [ "$currCell" == "" ]; then
    continue;
  fi
