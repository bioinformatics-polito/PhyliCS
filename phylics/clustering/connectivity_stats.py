...
def main():
    ...
    stats_df.to_csv(os.path.join(args.out_path, "connectivity_stats.csv"))

    #Plot mean scores + std deviations
    plt.suptitle("Connectivity Index and standard deviation\n",fontsize=16)
    sns.barplot(data=df)
    plt.gcf().set_size_inches(25, 12)
    plt.xlabel('clusterings')
    plt.ylabel('Mean connectivity index')
    plt.savefig(os.path.join(args.out_path, "connectivity_means.png"))
    plt.clf()
    ...
    fig.savefig(os.path.join(args.out_path, "connectivity_violin.png"))
    fig.clf()
