...
def main():
    ...
    stats_df.to_csv(os.path.join(args.out_path, "silhouette_stats.csv"))

    #Plot mean scores + std deviations
    plt.suptitle("Mean Silhouette Width and standard deviation\n",fontsize=16)
    sns.barplot(data=df)
    plt.gcf().set_size_inches(25, 12)
    plt.xlabel('clusterings')
    plt.ylabel('mean silhouette score')
    plt.savefig(os.path.join(args.out_path, "silhouette_means.png"))
    plt.clf()
    ...
    fig.savefig(os.path.join(args.out_path, "silhouette_violin.png"))
    fig.clf()
