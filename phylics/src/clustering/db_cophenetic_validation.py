...
def DBCV(X, labels, cophenetic_matrix):
    """
    ...
    """
    graph = _mutual_reach_dist_graph(X, labels, cophenetic_matrix)
    mst = _mutual_reach_dist_MST(graph)
    cluster_validity = _clustering_validity_index(mst, labels)
    return cluster_validity


def _core_dist(point, neighbors, cophenetic_matrix):
    """
    ...
    distance_vector = cophenetic_matrix.loc[point : neighbors)
...
def _mutual_reachability_dist(point_i, point_j, neighbors_i,
    ...
    """
    core_dist_i = _core_dist(point_i, neighbors_i, cophenetic_matrix)
    core_dist_j = _core_dist(point_j, neighbors_j, cophenetic_matrix)
    dist = cophenetic_matrix(point_i, point_j)
    mutual_reachability = np.max([core_dist_i, core_dist_j, dist])
    return mutual_reachability


def _mutual_reach_dist_graph(X, labels, cophenetic_matrix):
    """
