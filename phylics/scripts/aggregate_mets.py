...
def collect_het_scores(root, samples):
    het_scores = pd.DataFrame(columns=["met"]) 
    prefix = os.path.join(root, "Met", "data")
    for sample in samples:
        het_score = pd.read_csv("{dir}/{sample}/primary_metastasis_postCNV/heterogeneity_scores.csv".format(dir=prefix, sample=sample), sep = "\t").iloc[0]["het_score"].astype(float)
        het_scores = het_scores.append({"met":het_score}, ignore_index=True)
    return het_scores 
...
def main():
    ...
    het_scores = collect_het_scores(args.input_dir, SAMPLES)
