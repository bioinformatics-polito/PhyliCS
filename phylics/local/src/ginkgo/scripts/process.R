...
maxPloidy   = 6
minBinWidth = 5
#main_dir="/home/mmontemurro/ginkgo-runs/ginkgo/scripts"
...
cm          = args[[8]]
dm          = args[[9]]
cp          = as.numeric(args[[10]])
ref         = args[[11]]
f           = as.numeric(args[[12]])
facs        = args[[13]]
sex         = as.numeric(args[[14]])
bb          = as.numeric(args[[15]])
...
#  pdf(paste(lab[k], "_dist.pdf", sep=""), width=30, height=7)
#  
...
#    geom_point(data=outliers, aes(x=x, y=y), shape=5, size=3) +
...
#    theme(plot.margin=unit(c(.5,1,.5,1.75),"cm")) +
...
#  pdf(paste(lab[k], "_counts.pdf", sep=""), width=25, height=15)
...
#
#  #Plot lorenz curves
#  pdf(paste(lab[k], "_lorenz.pdf", sep=""), width=25, height=15)
#
#  nReads=sum(raw[,k])
...
#
#  #Plot GC correction
#  pdf(paste(lab[k], "_GC.pdf", sep=""), width=2500, height=1250)
#
...
#  pdf(paste(lab[k], "_hist.pdf", sep=""), width=25, height=15)
#
...
#  pdf(paste(lab[k], "_SoS.pdf", sep=""), width=25, height=15)
#
...
#  pdf(paste(lab[k], "_CN.pdf", sep=""), width=30, height=7.5)
#
#  top=8
#  rectangles1=data.frame(pos[seq(1,nrow(pos), 2),])
#  rectangles2=data.frame(pos[seq(2,nrow(pos), 2),])
...
#
#
#save.image("copynumber.Rdata")
#write("Making heatRaw.pdf", stderr())
##pdf("heatRaw.pdf", width=20, height=14)
...
##dev.off()
#
#write("Making heatNorm.pdf", stderr())
#step=quantile(fixedBPs, c(1))[[1]]
##pdf("heatNorm.pdf", width=20, height=14)
...
##dev.off()
#
#write("Making heatCN.pdf", stderr())
##step=min(20, quantile(finalBPs, c(1))[[1]])
##pdf("heatCN.pdf", width=20, height=14)
...
##dev.off()
#
#write("Making heatCor.pdf", stderr())
##pdf("heatCor.pdf", width=20, height=14)
